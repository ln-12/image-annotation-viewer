...
fun java.io.File.toProjectFile(): File = object : File {
    override val name: String get() = if(this@toProjectFile.name.isEmpty()) { this@toProjectFile.absolutePath } else { this@toProjectFile.name }
    override val path: Path = this@toProjectFile.toPath()
    override val isDirectory: Boolean get() = this@toProjectFile.isDirectory

    override val children: List<File>
        get() = this@toProjectFile
            .listFiles { file, name -> !name.startsWith(".") && (file.isDirectory || name.endsWith(".png")) }
            .orEmpty()
